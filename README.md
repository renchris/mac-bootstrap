...
```bash
curl -fsSLo /tmp/mac-bootstrap.sh https://raw.githubusercontent.com/renchris/mac-bootstrap/97ae35960cec28bfc3b792e64fce980e5b33a13f/bootstrap.sh && echo "2eb42da271c22385ec3f812a4fa96e48cb7aec3c65d0847cc1d932fad77b641b  /tmp/mac-bootstrap.sh" | shasum -a 256 -c - && bash /tmp/mac-bootstrap.sh
```
...
<picture>
  ...
  <img src="assets/diagrams/module-selection-light.svg" alt="--profile full includes --profile standard, which includes --profile lite (the default). lite installs agent_cli (Claude Code and Copilot CLI), statusline, instructions, hooks, skills, agent_env and pane_equalize; standard adds github_cli, handoff, microsoft365, rewrite_model and reporting_off; full adds voiceink, screenshot, microsoft365_archive, shared_folders, browser_automation and extra_agents (the Codex and Gemini CLIs). handoff needs statusline and hooks, microsoft365_archive needs microsoft365, and browser_automation needs agent_cli; each is added out loud when missing.">
</picture>
...
Replace `97ae35960cec28bfc3b792e64fce980e5b33a13f` with the release commit — never `main`, whose
raw URL serves up to five minutes of stale CDN bytes (`cache-control: max-age=300`, measured). At
that pinned commit `bootstrap.sh` is 1489 lines and `shasum -a 256` reads
`2eb42da271c22385ec3f812a4fa96e48cb7aec3c65d0847cc1d932fad77b641b`; step 1 checks it,
...
1. FETCH — never pipe a script into a shell. Save it and check it; the check must print OK:
     curl -fsSL -o /tmp/mac-bootstrap.sh https://raw.githubusercontent.com/renchris/mac-bootstrap/97ae35960cec28bfc3b792e64fce980e5b33a13f/bootstrap.sh
     echo "2eb42da271c22385ec3f812a4fa96e48cb7aec3c65d0847cc1d932fad77b641b  /tmp/mac-bootstrap.sh" | shasum -a 256 -c -
   If raw.githubusercontent.com cannot be reached, this is the one other source, under the same check:
     curl -fsSL -H 'Accept: application/vnd.github.raw' -o /tmp/mac-bootstrap.sh 'https://api.github.com/repos/renchris/mac-bootstrap/contents/bootstrap.sh?ref=97ae35960cec28bfc3b792e64fce980e5b33a13f'
   A third source serving the same bytes: https://cdn.jsdelivr.net/gh/renchris/mac-bootstrap@97ae35960cec28bfc3b792e64fce980e5b33a13f/bootstrap.sh
...
```bash
curl -fsSL -H 'Accept: application/vnd.github.raw' -o /tmp/mac-bootstrap.sh 'https://api.github.com/repos/renchris/mac-bootstrap/contents/bootstrap.sh?ref=97ae35960cec28bfc3b792e64fce980e5b33a13f' && echo "2eb42da271c22385ec3f812a4fa96e48cb7aec3c65d0847cc1d932fad77b641b  /tmp/mac-bootstrap.sh" | shasum -a 256 -c - && bash /tmp/mac-bootstrap.sh
```
...
| **`relaunch-kitty`** | Relaunch kitty — `allow_remote_control` and `listen_on` are read at startup, and only a restart creates the socket. `kitty @ load-config` cannot deliver this one, and on a kitty that never had remote control it cannot be delivered at all | Quit and reopen kitty, then `echo $KITTY_LISTEN_ON` in the new window — a path means `/handoff` can drive it | `pane_equalize`, and `handoff`'s kitty driver entirely |
| **`gh-sign-in`** | **Signing in to GitHub** — a browser flow that mints a token only you can create | `$HOME/.mac-bootstrap/tools/bin/gh auth login`, adding `--hostname <host>` when `GH_HOST` names a GitHub Enterprise tenant | `github_cli` |
| **`extra-agent-sign-in`** | **Signing in to Codex and to Gemini**, one browser flow each, to two different vendors | the command the receipt prints for each CLI it installed | `extra_agents` |
| **`a-browser-to-drive`** | **A Chromium-family browser** — this Mac has none, and nothing here installs one: it is a drag into `/Applications`, which is IT's to distribute | Ask IT for Google Chrome (Microsoft Edge, Chromium or Brave do as well) | `browser_automation` |
...
|---|---|---|---|---|
| `skills` | three portable skills both agents load | **DELIVERED — path documented, not probed** | **DELIVERED — measured** | Handing a session on, keeping a plan document, and verifying by independent read-back: one `SKILL.md` each, the same bytes for both agents. Copilot's personal tier is measured rather than generalised — `copilot skill --help` names `~/.copilot/skills/`, and after a sandbox install `copilot skill list` read all three back. Claude Code's `~/.claude/skills/` is its documented default, the tier `instructions` already writes into, and is not probed here because a live read-back needs a signed-in agent. Files land **only** into an agent root that already exists, so an agent this Mac does not have is never made to look configured. A skill of somebody else's at one of the three names is never overwritten and never removed: the collision is `NEEDS_HUMAN` with a `diff`. A company policy that reserves personal skills to IT is `NEEDS_HUMAN` naming the key — never a green over skills that will not load. |
| `agent_env` | the environment the agent runs in | **DELIVERED** | **nothing to write** | Keys in Claude Code's settings `env`, which it applies to every session and every process it starts. `MCP_TIMEOUT` 60000 doubles the documented 30 s a server gets to start — where a TLS-inspecting proxy plus a first-run fetch stops looking slow and starts looking dead. `MCP_TOOL_TIMEOUT` 600000 is explicit because the two documents disagree about its default; one server that needs longer keeps its own `timeout` in `.mcp.json`, which overrides it. `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH` is **capability-detected**: it appears in no documented reference and in no Claude Code package measured here, so it is written only when the agent this Mac runs carries the name, and otherwise the row says so rather than setting something nothing reads. `DISABLE_AUTOUPDATER` is opt-in, written only when `BOOTSTRAP_PIN_AGENT_VERSION` is set, because freezing an agent version has a security tail and is your decision. A key IT pins in managed settings is `NEEDS_HUMAN`, never a green over a write that is ignored. |
| `github_cli` | the GitHub CLI (`gh`) | **DELIVERED — after your sign-in** | same — agent-independent | `gh` in your home folder: no admin, no Homebrew, pinned and kept only when its sha256 and GitHub's signing Team ID (`VEKTX9H2N7`) match. A `gh` already on the Mac is used as it is. Signing in is yours — one `gh auth login` in your browser, `--hostname` when `GH_HOST` names a GitHub Enterprise tenant, because a bare `gh auth status` exits 0 when *any* host is signed in and would mask an unsigned Enterprise one. A blocked download names the host and `BOOTSTRAP_ARTIFACT_MIRROR`; a binary this Mac refuses to run names the signer for IT. Uninstall never touches your `gh` config, keychain item or git credential helper. |
| `browser_automation` | the `agent-browser` CLI | **DELIVERED — when this Mac has a browser** | same — agent-independent | So an agent can open a page, click, fill a form, take a screenshot and read the page back — driving the Chrome, Edge, Chromium or Brave this Mac already has, named explicitly rather than left to the CLI's own search, which knows neither Edge nor `~/Applications`. Installs **no** browser: with none, it is `NEEDS_HUMAN` and says plainly that there is nothing to drive. Runs on the same pinned node `microsoft365` fetches, from a module-private prefix, never globally and never with `sudo`; the package's postinstall, which re-downloads the same binary from a third host, is not run. At run time it reaches whatever host you drive it to, and it drives a browser holding your logged-in sessions — both stated in `--egress` and in what IT governs. |
| `extra_agents` | the other two coding-agent CLIs | **DELIVERED — after your sign-in** | same — agent-independent | OpenAI's Codex CLI and Google's Gemini CLI, pinned and hash-checked into your home folder, no admin and no global npm, each independent so one being blocked never fails the other. Codex is the vendor's release tarball, refused unless it is signed by Team ID `2DC432GLL2`. Gemini is its own npm tarball run by the pinned node — **its standalone macOS binaries are literally unsigned and are killed on sight by Apple silicon**, so they are not installable at any hash. Each CLI sends your work to **its own vendor**, which is a second and third model provider on this Mac: a company may sanction one agent and not another, so they are declared separately. Asking either its version is not a free read — `gemini --version` alone creates `~/.gemini` — so every probe runs under a throwaway home and the looking modes write nothing. |
