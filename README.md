...
Replace `34eb77b7f114dda0a3ef9513701fa727e2688c42` with the release commit — never `main`, whose
raw URL serves up to five minutes of stale CDN bytes (`cache-control: max-age=300`, measured). At
that pinned commit `bootstrap.sh` is 829 lines and `shasum -a 256` reads
`503479a9b210858d86746f25252df9cb439fa83e3007b0db07d1c3aaa49decc8`; step 1 shows you the checksum,
...
1. FETCH — never pipe a script into a shell. Save it, then show me its checksum and first lines:
     curl -fsSL -o /tmp/mac-bootstrap.sh https://raw.githubusercontent.com/renchris/mac-bootstrap/34eb77b7f114dda0a3ef9513701fa727e2688c42/bootstrap.sh
