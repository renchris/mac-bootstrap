...
| `BOOTSTRAP_ARTIFACT_MIRROR` | the operator | a company mirror, or a carried folder (`file://`), for every **third-party** download a module pins — node, ollama, Hammerspoon, iTerm2, kitty, the agents. It holds the same bytes at `<mirror>/<host>/<path>` (the layout `wget -x` writes), and `bootstrap_fetch_pinned` tries it first, then the vendor, both under the sha256 pinned in the module — so the mirror is trusted for nothing. |
| `BOOTSTRAP_AGENTS` | the operator · tests | which coding agents `agent_cli` installs into `$HOME`: `claude copilot` (the default), either alone, or `none` — the Mac's agents are IT's, so install nothing and probe nothing. Any other word is FAILED with the valid set named. `scripts/characterize.sh` exports `none`, so no suite run downloads an agent or reads a real sign-in. |
| `BOOTSTRAP_AGENT_CLI_{CLAUDE,COPILOT}_{URL,SHA256}` · `BOOTSTRAP_AGENT_CLI_CODESIGN` · `BOOTSTRAP_AGENT_CLI_SECURITY` | tests | a `file://` fixture and its hash in place of the vendor's pinned artifact, and stand-ins for `codesign` and `security`, whose signature and keychain answers a fixture cannot produce. |
| `BOOTSTRAP_TARBALL` · `BOOTSTRAP_GIT_URL` · `BOOTSTRAP_JSDELIVR` · `BOOTSTRAP_GITHUB_API` | tests | replace the tarball URL, the git remote, the jsDelivr base (`…/gh/<repo>@<pin>`) and the GitHub API base of the five tree routes, which a curl'd run tries in that order after BOOTSTRAP_RAW's raw host — tarball, git, raw, jsDelivr, contents API — each under the manifest. A mirror named in BOOTSTRAP_RAW is used on its own. |
...
| `BOOTSTRAP_MICROSOFT_SERVER` | the operator | the Softeria server `microsoft365_archive` reads through: an absolute path to its `dist/index.js` or its bin link. Unset, the archive takes `microsoft365`'s install, else the `ms365` server already registered in `$HOME/.claude.json` — read, never written, with that registration's tenant and client id (`common` when it names no tenant) and the node beside its bin link. A candidate counts only when its real path is Softeria's `dist/index.js` AND running it answers `--version` with a semver. Recorded at install; a named server that does not run is reported as a gate, never swapped for another. |
