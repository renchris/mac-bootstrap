#!/bin/bash
# m5_panes — Cmd+Shift+E evens out split panes, in kitty AND iTerm2.
#
...
M5_MARK_BEGIN='# >>> mac-bootstrap m5_panes >>>'
M5_MARK_END='# <<< mac-bootstrap m5_panes <<<'
M5_KEYEQ='@$e'                                    # single quotes: this is literal, not $e
M5_ITERM_SELECTOR='arrangeSplitPanesEvenly:'
M5_ITERM_BOGUS='definitelyNotASelector_zzz:'      # the negative arm of the capability probe
M5_ITERM_ARRANGE='Arrange Split Panes Evenly'
M5_ITERM_TIMESTAMPS='Show Timestamps'
M5_ITERM_CHORD='0x45-0x120000'                    # Cmd+Shift+E as iTerm2 records it
M5_KITTY_FLOOR='0.47.2'                           # first release with `layout_action equalize`
M5_PLISTBUDDY='/usr/libexec/PlistBuddy'

# ── small helpers ────────────────────────────────────────────────────────────────────────────

# _m5_ver_ge <have> <want> — dotted numeric compare, three components, bash 3.2.
...
# _m5_field <text> <key> — prints the value of a KEY=VALUE line, rc 1 if absent.
_m5_field() {
  local line k="${2:-}"
...
_m5_iterm_app() {                                 # prints the app bundle, rc 1 if absent
  local c
...
_m5_iterm_plist() { printf '%s' "$HOME/Library/Preferences/com.googlecode.iterm2.plist"; }
_m5_iterm_dyndir() { printf '%s' "$HOME/Library/Application Support/iTerm2/DynamicProfiles"; }

# _m5_iterm_capable <app> — does THIS build ship the menu item? R2: base-system grep only.
...
# _m5_conflict — prints "dyn <path>" or "plist", rc 0 when something else already owns the
...
_m5_conflict() {
  ...
  if [ -d "$d" ]; then
    for j in "$d"/*; do
      [ -f "$j" ] || continue
      LC_ALL=C grep -qa "\"$M5_ITERM_CHORD\"" "$j" 2>/dev/null && { printf 'dyn %s' "$j"; return 0; }
    done
  fi
  p="$(_m5_iterm_plist)"
  [ -f "$p" ] || return 1
...
# _m5_pb_set <plist> <menu item title> <key equivalent>
_m5_pb_set() {
  local p="${1:-}" k="${2:-}" v="${3:-}"
  "$M5_PLISTBUDDY" -c "Add :NSUserKeyEquivalents:'$k' string '$v'" "$p" >/dev/null 2>&1 && return 0
  "$M5_PLISTBUDDY" -c "Set :NSUserKeyEquivalents:'$k' '$v'" "$p" >/dev/null 2>&1
}
...
# _m5_iterm_install — PlistBuddy writes; the format is restored afterwards. PlistBuddy rewrites
...
_m5_kitty_ver() {                                 # prints "0.48.2", rc 1 if unreadable
  local out v
...
_m5_kitty_ver_ok() {
  local v
  v="$(_m5_kitty_ver "${1:-}")" || return 1
  _m5_ver_ge "$v" "$M5_KITTY_FLOOR"
}
...
_m5_kitty_probe() {                               # prints the KEY=VALUE block, rc 1 if it could not run
  local kb td out rc
  kb="$(_m5_kitty_bin)" || return 1
  ...
  [ "$(_m5_field "$out" CTL_EQ)" = yes ] || return 1
  [ "$(_m5_field "$out" CTL_LAYOUT0)" = splits ] || return 1
  [ "$(_m5_field "$out" NEG_EQ)" = no ] || return 1
  printf '%s' "$out"
}
...
_m5_kitty_layout_line() {
  printf 'enabled_layouts splits:equalize_on_window_close=true%s' \
    "${1:+,$1}"
}

# _m5_kitty_confpath — kitty's own answer when the probe runs; its documented lookup when not.
_m5_kitty_confpath() {
  local out d
  out="${1:-}"
  d="$(_m5_field "$out" CONFDIR)" || d=""
  if [ -z "$d" ]; then
...
_m5_kitty_verify() {
  ...
  [ "$(_m5_field "$out" USR_EQ)" = yes ] || return 1
  return 0
}

# _m5_kitty_strip <conf> — remove our block, refusing on any shape but exactly one begin and
...
_m5_kitty_strip() {
  ...
  [ -f "$conf" ] || return 0
  nb="$(LC_ALL=C grep -c "^$M5_MARK_BEGIN\$" "$conf" 2>/dev/null)"
  ne="$(LC_ALL=C grep -c "^$M5_MARK_END\$" "$conf" 2>/dev/null)"
  ...
  awk -v b="$M5_MARK_BEGIN" -v e="$M5_MARK_END" '
    $0 == b { skip = 1; next }
    $0 == e { skip = 0; next }
    !skip   { print }
...
_m5_kitty_install() {
  ...
  eq="$(_m5_field "$out" USR_EQ)"
  layouts="$(_m5_field "$out" USR_LAYOUTS)"
  ...
  [ "$need_layout" = 0 ] && [ "$need_map" = 0 ] && return 0
  {
    ...
    if [ "$need_layout" = 1 ]; then
      ...
      rest="$(_m5_kitty_rest_layouts "$layouts")"
...
# _m5_kitty_rest_layouts <comma list> — everything except splits, order kept, opts kept.
_m5_kitty_rest_layouts() {
...
_m5_gate_reason() {
  local app="" kb="" c
  if app="$(_m5_iterm_app)"; then
    _m5_iterm_capable "$app" || { printf 'ITERM_OLD'; return 0; }
  fi
  if kb="$(_m5_kitty_bin)"; then
    _m5_kitty_ver_ok "$kb" || { printf 'KITTY_OLD'; return 0; }
  fi
...
verify_m5_panes() {
  ...
  if _m5_iterm_app >/dev/null 2>&1; then
    _m5_iterm_verify || return 1
    configured=1
  fi
  if _m5_kitty_bin >/dev/null 2>&1; then
    _m5_kitty_verify || return 1
    configured=1
  fi
...
note_m5_panes() {
  case "$(_m5_gate_reason)" in
    ...
    KITTY_OLD)
      printf '%s' "kitty is older than $M5_KITTY_FLOOR, the first release with the 'equalize' layout action" ;;
    CONFLICT_DYN)
...
install_m5_panes() {
  local did=0 rc=0
  if _m5_iterm_app >/dev/null 2>&1; then
    if _m5_iterm_install; then
      did=1
      printf '%s\n' "m5_panes: iTerm2 — Cmd+Shift+E bound to Arrange Split Panes Evenly."
      printf '%s\n' "m5_panes: it takes effect at the NEXT iTerm2 launch; the running instance ignores it."
    else
      rc=1
    fi
  fi
  if _m5_kitty_bin >/dev/null 2>&1; then
    if _m5_kitty_install; then
      did=1
      printf '%s\n' "m5_panes: kitty — cmd+shift+e mapped to layout_action equalize, splits made the default layout."
      printf '%s\n' "m5_panes: a running kitty picks it up on restart, or with: kitty @ --to \"\$KITTY_LISTEN_ON\" load-config"
    else
      rc=1
    fi
  fi
  ...
  printf '%s\n' "m5_panes: CHECK IT BY HAND — open the terminal, split the pane twice, drag a divider, press Cmd+Shift+E."
  [ "$did" = 1 ] || return 1
  return "$rc"
}

uninstall_m5_panes() {
  _m5_iterm_app >/dev/null 2>&1 && _m5_iterm_uninstall
  _m5_kitty_bin >/dev/null 2>&1 && _m5_kitty_uninstall
  return 0
}
