# Patch for `pb-stop.sh` — ARM R, the self-recycle block

**Owner of the file being patched: m3.** m4 owns `agent-handoff`, `handoff.md` and
`m4_handoff.sh` and does not edit `assets/hooks/pb-stop.sh`, so the arm arrives as this document
...
`pb-stop.sh`'s arm B renders the context advisory into **`systemMessage`**, and that is correct —
...
The harness is `pb-stop.sh`'s own skeleton (B1 + the counter + a stub arm A) with the block above
