...
**Owner of the file being patched: hooks.** handoff owns `agent-handoff`, `handoff.md` and
`handoff.sh` and does not edit `assets/hooks/stop.sh`, so the arm arrives as this document
instead. Everything it needs already exists: `agent-handoff recycle-due` is installed by handoff and
...
#
# ALL THE JUDGMENT LIVES IN `agent-handoff recycle-due`, which is handoff's and is fixture-tested
...
spliced in at the position above, driven by real telemetry files and a real `install_handoff`.
...
It fires **once per session**, from a fill number **someone else** measured. If statusline's statusline is
