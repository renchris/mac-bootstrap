...
# still quoted elsewhere: "with no jq this hook is FULLY INERT". That WAS true — pb_json refuses a
# nested path without jq, so $CMD came back empty and every command was allowed. It is fixed here,
# not by a better regex but by using the other engine the library already trusts: pb3_field writes
# the payload to a temp file and reads it with plutil, which parses nesting and quoting properly.
# Measured: the whole 66-case matrix passes under PB_NO_JQ=1, and the selftest carries a no-jq arm
...
#
# Seams: PB_BASH_GUARD=0 disables. Self-test: bash pb-guard-bash.sh --selftest
set -u
...
# ── pb3_segments — split a command into judgeable segments, quote-aware. ─────────────────────
...
# ── pb3_check_segment <segment> → 0 = deny (reason in PB3_REASON) · 1 = nothing to say ───────
pb3_check_segment() {
  ...
  if [ "$cw" = rm ] && _hasr && _hasf && { [ "$tgt" = 1 ] || [ "$root" = 1 ]; }; then
    PB3_REASON="DENIED: a recursive+force rm naming \$HOME or /. Flag spelling is irrelevant — -rf, -fr, -Rf, -r -f and --recursive --force are the same command, and quoting the path changes nothing. Name a path relative to the project root instead."
    return 0
  fi
  # 2. git clean -x / -X — deletes GITIGNORED files
  if [ "$cw" = git ] && [ "$sub" = clean ]; then
    case "$flags" in *[xX]*)
      PB3_REASON="DENIED: 'git clean' with -x/-X deletes GITIGNORED files — .env, credentials, build caches, and any generated or paid asset that is ignored on purpose. Preview with 'git clean -nd', then use 'git clean -fd' (no -x)."
      return 0 ;;
    esac
  fi
  # 3. --no-verify / git commit -n
  case "$longs" in *--no-verify*)
    PB3_REASON="DENIED: --no-verify bypasses the pre-commit hook. If a hook blocked the commit it caught something real — fix the cause."
    return 0 ;;
  esac
  if [ "$cw" = git ] && [ "$sub" = commit ]; then
    case "$flags" in *n*)
      PB3_REASON="DENIED: 'git commit -n' is the short form of --no-verify and bypasses the pre-commit hook. Fix the cause instead."
      return 0 ;;
    esac
  fi
  ...
  if [ "$cw" = git ] && [ "$sub" = push ]; then
    if [ "$plus" = 1 ]; then
      PB3_REASON="DENIED: a leading '+' in a push refspec IS a force push, with no --force anywhere in the command, and this one targets the trunk. Push a topic branch and open a PR, or ask the operator explicitly."
      return 0
    fi
    if _hasf && [ "$trunk" = 1 ]; then
      PB3_REASON="DENIED: force-push touching the trunk. This rewrites history other clones already have. Push a topic branch and open a PR, or ask the operator explicitly. (--force-with-lease is the safe form and is allowed.)"
      return 0
    fi
  fi
  # 5. git add -f — force-add a gitignored path
  if [ "$cw" = git ] && [ "$sub" = add ] && _hasf; then
    PB3_REASON="DENIED: 'git add -f' force-adds a file .gitignore excludes. If it is ignored that was deliberate — force-adding bloats history and defeats the protection."
    return 0
  fi
  return 1
}

# ── pb3_verdict <command-string> → prints the deny JSON and returns 0, or prints nothing / 1 ──
...
'
  ...
  set -- $(printf '%s' "$cmd" | pb3_segments)
  IFS="$oldifs"; [ "$hadf" = 1 ] || set +f
  for s in "$@"; do
    [ -n "$s" ] || continue
    if pb3_check_segment "$s"; then rc=0; break; fi
  done
  [ "$rc" = 0 ] || return 1
  jq="$(pb_jq)" || jq=""
  if [ -n "$jq" ]; then
    "$jq" -nc --arg r "$PB3_REASON" \
    ...
    printf '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"%s"},"permissionDecision":"deny","permissionDecisionReason":"%s"}\n' \
      "$(pb_json_escape "$PB3_REASON")" "$(pb_json_escape "$PB3_REASON")"
  fi
  return 0
}
...
pb3_selftest() {
  ...
  printf '\n-- INSTRUMENT CONTROLS --\n'
  n=$((n+1))
  if [ -z "$(pb3_verdict '')" ]; then printf '  ok   an empty command says nothing\n'
  else bad=$((bad+1)); printf '  FAIL empty command produced a verdict\n'; fi
  n=$((n+1))
  out="$(printf '{"tool_name":"Bash","tool_input":{"command":"rm -rf \\"$HOME/x\\""}}' | /bin/bash "$PB3_SELF" 2>/dev/null)"
  ...
  n=$((n+1))
  printf '{"tool_name":"Bash","tool_input":{"command":"rm -rf \\"$HOME/x\\""}}' | /bin/bash "$PB3_SELF" >/dev/null 2>&1
  ...
  n=$((n+1))
  out="$(printf '{"tool_name":"Bash","tool_input":{"command":"rm -rf \\"$HOME/x\\""}}' | PB_NO_JQ=1 /bin/bash "$PB3_SELF" 2>/dev/null)"
  ...
  n=$((n+1))
  out="$(printf '%s' '{"tool_name":"Bash","tool_input":{"command":"ls"}}' | /bin/bash "$PB3_SELF" 2>/dev/null)"
...
Wire it with the repo's own writer, never by hand-editing a settings file:
  ...
  pb_hook_wire         "$HOME/.claude/settings.json"            PreToolUse Bash "$HOME/.mac-bootstrap/hooks/pb-guard-bash.sh" 10
  pb_copilot_hook_wire "$HOME/.copilot/hooks/00-lifecycle.json" PreToolUse Bash "$HOME/.mac-bootstrap/hooks/pb-guard-bash.sh" 10
...
[ "${PB_BASH_GUARD:-1}" = 1 ] || exit 0
PB3_IN="$(cat 2>/dev/null || true)"
PB3_CMD="$(pb3_field "$PB3_IN" tool_input.command)"
[ -n "$PB3_CMD" ] || exit 0
pb3_verdict "$PB3_CMD" || true
exit 0
