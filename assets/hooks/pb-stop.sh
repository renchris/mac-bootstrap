...
#   THE RULE NOW: arm B is computed by pb_ctx_advisory, which reads NO git, NO repo root and NO
...
#   B2  a per-session counter at $PB_STATE_DIR/stop-count, ceiling PB_STOP_MAX (default 3).
...
# older than PB_STOP_TTL_DAYS (default 7) are reaped, so the store cannot grow without bound.
#
...
# field, so there they ride stderr instead — see pb3_agent().
#
# Seams: PB_STOP=0 disables · PB_STOP_MAX · PB_CTX_T · PB_STATE_DIR · PB_TELEMETRY_DIR.
...
# ── pb3_agent <transcript_path> <stop_reason> → claude | copilot ─────────────────────────────
...
# ── pb3_say <agent> <line> — arm A/B output. Never extends the turn. ─────────────────────────
pb3_say() {
  local jq
...
# ── pb3_block <agent> <reason> <banner> — the ONE blocking output. ───────────────────────────
pb3_block() {
  local jq
  jq="$(pb_jq)" || jq=""
  if [ -n "$jq" ]; then
...
pb3_selftest() {
  ...
  _run() {   # _run <cwd> <payload-extra> → the hook's stdout, with a private state dir
    ( cd "$1" 2>/dev/null || exit 0
      PB_STATE_DIR="$T/state" PB_TELEMETRY_DIR="$T/tel" \
      /bin/bash "$PB3_SELF" <<XIN 2>/dev/null
...
XIN
  ...
  _tel 82
  out="$( cd "$T/bare" && PB_TELEMETRY_DIR="$T/tel" /bin/bash -c '
      . "$1/pb-lib.sh" || exit 0
      ROOT="$(pb_git "$PWD" rev-parse --show-toplevel)"
      LEDGER=""; [ -n "$ROOT" ] && LEDGER="clean"
      CTX="$(pb_ctx_advisory "$2")"
      if [ -n "$LEDGER" ]; then printf "%s%s\n" "$LEDGER" "$CTX"; fi
    ' _ "$PB3_SELF_DIR" "$sid" 2>/dev/null )"
  ...
  _shimrun() {
    ( cd "${1:-$T/bare}" 2>/dev/null || exit 0
      PATH="$T/shim:$PATH" PB_STATE_DIR="$T/state" PB_TELEMETRY_DIR="$T/tel" \
      /bin/bash "$PB3_SELF" <<XIN 2>/dev/null
...
XIN
  ...
  _tel 82
  out="$( cd "$T/bare" && PB_STATE_DIR="$T/state" PB_TELEMETRY_DIR="$T/tel" \
          /bin/bash "$PB3_SELF" <<XIN 2>/dev/null
...
XIN
  ...
  # fixed by pb3_field, and pinned here: without this case the whole suite is green on the arm
  # that matters least.
  out="$( cd "$T/bare" && PB_NO_JQ=1 PB_STATE_DIR="$T/state" PB_TELEMETRY_DIR="$T/tel" \
          /bin/bash "$PB3_SELF" <<XIN 2>/dev/null
{"session_id":"$sid","cwd":"$T/bare","stop_hook_active":true}
XIN
       )"
...
XIN
  ...
  # ── B2: arm C blocks at most PB_STOP_MAX times, and abstains on someone else's file ────────
  if [ -n "$repo" ] && pb_have_jq; then
    ...
    f=0; i=1
    while [ "$i" -le 4 ]; do
      out="$( cd "$repo" && PB_STATE_DIR="$T/state" PB_TELEMETRY_DIR="$T/tel" PB_STOP_MAX=3 \
              /bin/bash "$PB3_SELF" <<XIN 2>/dev/null
...
XIN
    ...
    out="$( cd "$repo" && PB_STATE_DIR="$T/state" PB_TELEMETRY_DIR="$T/tel" \
            /bin/bash "$PB3_SELF" <<XIN 2>/dev/null
...
# ── B1 ────────────────────────────────────────────────────────────────────────────────────────
[ "$(pb3_field "$IN" stop_hook_active)" = "true" ] && exit 0

SID="$(pb3_field "$IN" session_id)"
TP="$(pb3_field "$IN" transcript_path)"
SR="$(pb3_field "$IN" stop_reason)"
CWD="$(pb3_field "$IN" cwd)"; [ -n "$CWD" ] || CWD="$PWD"
AGENT="$(pb3_agent "$TP" "$SR")"
...
if [ -n "$ROOT" ]; then
  BRANCH="$(pb_git "$ROOT" rev-parse --abbrev-ref HEAD)"
  PORC="$(pb_git "$ROOT" status --porcelain)"
  DIRTY_N="$(printf '%s' "$PORC" | pb_count)"
  TRUNK="$(pb_trunk "$ROOT")"
  if [ -n "$TRUNK" ]; then AHEAD="$(pb_git "$ROOT" rev-list --count "$TRUNK..HEAD")"; fi
  case "$DIRTY_N" in ''|*[!0-9]*) DIRTY_N=0 ;; esac
  case "$AHEAD"   in ''|*[!0-9]*) AHEAD=0 ;; esac
  # THREE RUNGS AND A FOURTH THAT SAYS "I CANNOT TELL". With no remote, pb_trunk returns empty and
...
CNT_F="$PB_STATE_DIR/stop-count"; MAX="${PB_STOP_MAX:-3}"; CNT=0
...
find "$PB_STATE_DIR" -maxdepth 1 -name 'stop-count' -mtime "+${PB_STOP_TTL_DAYS:-7}" -delete 2>/dev/null

MINE=""
if MINE="$(pb3_mine_dirty)"; then :; else MINE=""; fi
...
printf '%s %s\n' "$RUNG" "${LEDGER:-no git repo at that cwd — nothing to report}" \
  > "$PB_STATE_DIR/last-ledger" 2>/dev/null || true
