...
#  3. NEVER writes an authorization surface. pb_settings_merge refuses a keypath or a file that
...
# pb_settings_merge stops a module that calls US; it cannot stop a module that calls plutil
...
PB_STATE_DIR="${PB_STATE_DIR:-$HOME/.mac-bootstrap}"
PB_TELEMETRY_DIR="${PB_TELEMETRY_DIR:-/tmp/pb-telemetry}"
PB_CTX_T="${PB_CTX_T:-70}"                 # context-fill threshold, percent
PB_CTX_MAX_AGE="${PB_CTX_MAX_AGE:-600}"    # telemetry older than this says NOTHING
PB_LIB_VERSION=1
PB_BACKED_UP=""                            # files this process has already backed up

mkdir -p "$PB_STATE_DIR" 2>/dev/null || true

PB_PLUTIL=/usr/bin/plutil

# ── pb_warn <text> — stderr + the log. NEVER stdout: a hook's stdout is parsed as JSON. ──────
pb_warn() {
  printf 'pb: %s\n' "$*" >&2
  [ -n "${PB_LOG:-}" ] && printf '%s pb-lib %s\n' "$(date -u +%FT%TZ)" "$*" >>"$PB_LOG" 2>/dev/null
  return 0
}

# ── pb_jq — prints an ABSOLUTE jq path, or returns 1. Absolute first (§C2): a PATH lookup in a
#    hook inherits whatever the agent's environment happens to be. PB_NO_JQ=1 forces the plutil
...
pb_have_jq() { pb_jq >/dev/null 2>&1; }

# ── pb_json_escape <text> — make text safe inside a JSON string literal. ─────────────────────
...
# ── pb_json_ok <file> — IS THIS VALID JSON? Two engines; both must agree. ────────────────────
...
pb_json_ok() {
  local f="${1:-}" jq
  [ -f "$f" ] || return 1
  "$PB_PLUTIL" -convert json -o /dev/null "$f" >/dev/null 2>&1 || return 1
  jq="$(pb_jq)" || return 0          # no jq ⇒ one engine is what we have; not a failure
  "$jq" -e . "$f" >/dev/null 2>&1 || return 1
  return 0
}

# ── pb_is_json_text <file> — is this file JSON *TEXT*, as opposed to some other plist format? ─
# pb_json_ok answers "can plutil parse this", and plutil parses XML and BINARY plists happily —
...
# ── pb_json_type <json-text> → plutil's type name, or nothing if the text is not valid JSON ──
...
pb_json_type() {
  ...
  out="$("$PB_PLUTIL" -type _v "$t" 2>/dev/null)"
  if [ $? -ne 0 ] || [ -z "$out" ] || [ "$out" = "(any)" ]; then rm -f "$t"; return 1; fi
  rm -f "$t" 2>/dev/null
  printf '%s' "$out"
}

# ── pb_json_fmt <json-text> → "json" for a container, "raw" for a scalar. ────────────────────
...
# ── pb_json_norm <json-text> — plutil's canonical rendering of a JSON value. ─────────────────
...
# Containers come back as JSON; scalars come back in `raw` form, which is what pb_settings_get
# will hand back for them too.
pb_json_norm() {
  local t fmt out rc
  fmt="$(pb_json_fmt "${1:-}")" || return 1
  t="$(mktemp -t pbnorm 2>/dev/null)" || return 1
  printf '{"_v":%s}' "${1:-}" > "$t" 2>/dev/null || { rm -f "$t"; return 1; }
  out="$("$PB_PLUTIL" -extract _v "$fmt" -o - "$t" 2>/dev/null)"
  rc=$?
  rm -f "$t" 2>/dev/null
  [ $rc -eq 0 ] || return $rc
  printf '%s' "$out"
...
# ── pb_settings_get <file> <keypath> [json|raw] — THE READ-BACK. plutil only, never jq. ──────
...
pb_settings_get() {
  ...
  [ -f "$f" ] || return 1
  out="$("$PB_PLUTIL" -extract "$k" "$fmt" -o - "$f" 2>/dev/null)"
  rc=$?
  [ $rc -eq 0 ] || return $rc
  printf '%s' "$out"
}

pb_settings_type() {    # prints plutil's type name for a keypath, or nothing (rc 1)
  local f="${1:-}" k="${2:-}" out rc
  [ -f "$f" ] || return 1
  out="$("$PB_PLUTIL" -type "$k" "$f" 2>/dev/null)"
  rc=$?
  [ $rc -eq 0 ] || return $rc
  printf '%s' "$out"
}

# pb_array_len <file> <keypath> → how many elements the array at keypath has (0 if absent).
pb_array_len() {
  local f="${1:-}" k="${2:-}" i=0
  [ -f "$f" ] || { printf '0'; return 0; }
  while [ "$i" -lt 256 ] && "$PB_PLUTIL" -extract "$k.$i" json -o - "$f" >/dev/null 2>&1; do
    i=$((i + 1))
  done
  printf '%s' "$i"
}

# ── pb_backup <file> — once per file per process, never overwritten. ─────────────────────────
pb_backup() {
  local f="${1:-}"
  case "$PB_BACKED_UP" in *"|$f|"*) return 0 ;; esac
  PB_BACKED_UP="$PB_BACKED_UP|$f|"
  [ -f "$f" ] || return 0
  cp -p "$f" "$f.pb-bak.$(date -u +%Y%m%dT%H%M%SZ)" 2>/dev/null || true
  return 0
}

# ── pb_keypath_json <dotted> — ["hooks","Stop",0]; numeric segments stay NUMBERS. ────────────
...
pb_keypath_json() {
  ...
  for seg in "$@"; do
    ...
    first=0
    case "$seg" in
      ''|*[!0-9]*) out="$out\"$(pb_json_escape "$seg")\"" ;;
      *)           out="$out$seg" ;;
    esac
  done
  printf '%s]' "$out"
...
# ── pb_settings_refuse <file> <keypath> — rule 3, at the chokepoint. ─────────────────────────
...
pb_settings_refuse() {
  ...
  case "$f" in
    *settings.local.json|*.credentials.json|*/.credentials*|*/keychain*)
      pb_warn "REFUSED: $f is an authorization/credential surface; ask the operator in chat."; return 0 ;;
  esac
  case "$k" in
    ...
    |enabledMcpjsonServers*|trust*|autoApprove*)
      pb_warn "REFUSED: '$k' authorizes the agent; ask the operator in chat."; return 0 ;;
  esac
  return 1
}

# ═════════════════════════════════════════════════════════════════════════════════════════════
# pb_settings_merge — THE ONE SETTINGS WRITER (C2). Nothing else in this repo writes a JSON
...
pb_settings_merge() {
  ...
  [ -n "$f" ] && [ -n "$k" ] && [ -n "$v" ] || { pb_warn "pb_settings_merge: need <file> <keypath> <json>"; return 2; }
  pb_settings_refuse "$f" "$k" && return 2
  ...
  fmt="$(pb_json_fmt "$v")"  || { pb_warn "pb_settings_merge: value is not valid JSON: $v"; return 2; }
  norm="$(pb_json_norm "$v")" || { pb_warn "pb_settings_merge: cannot normalise: $v"; return 2; }

  mkdir -p "$(dirname "$f")" 2>/dev/null || true

  if [ "$mode" = "set" ]; then
    cur="$(pb_settings_get "$f" "$k" "$fmt" 2>/dev/null)" || cur=""
    [ -n "$cur" ] && [ "$cur" = "$norm" ] && return 0        # already exactly this — no write
  fi
  idx="$(pb_array_len "$f" "$k")"                            # where an append will land

  pb_backup "$f"
  tmp="$f.pb-tmp.$$"
  if [ -s "$f" ]; then
    cp -p "$f" "$tmp" 2>/dev/null || { pb_warn "cannot copy $f"; return 2; }
    pb_json_ok "$tmp" || { rm -f "$tmp"; pb_warn "$f is not valid JSON — refusing to touch it."; return 2; }
    pb_is_json_text "$tmp" || { rm -f "$tmp"; pb_warn "$f parses, but it is an XML or binary plist, not JSON text — refusing to touch it."; return 2; }
  else
    printf '%s\n' '{"_pbseed":1}' > "$tmp" 2>/dev/null || { pb_warn "cannot write $tmp"; return 2; }
    seeded=1
  fi

  jq="$(pb_jq)" || jq=""
  if [ -n "$jq" ]; then
    path="$(pb_keypath_json "$k")"
    if [ "$mode" = "append" ]; then
  ...
  if [ -z "$jq" ]; then                                        # ── the plutil arm ──
    ...
    if [ "$(pb_json_norm "$(cat "$tmp" 2>/dev/null)" 2>/dev/null)" = "{}" ]; then
      printf '%s\n' '{"_pbseed":1}' > "$tmp"; seeded=1
    fi
    pb_settings_ensure_parents "$tmp" "$k" || { rc=$?; rm -f "$tmp"; return "$rc"; }
    if [ "$mode" = "append" ]; then
      ...
      if [ "$(pb_settings_type "$tmp" "$k" 2>/dev/null)" = "array" ]; then
        "$PB_PLUTIL" -insert "$k" -json "$v" -append "$tmp" >/dev/null 2>&1 || {
          rm -f "$tmp"; pb_warn "plutil -append failed at $k"; return 2; }
      else
        "$PB_PLUTIL" -replace "$k" -json "[$v]" "$tmp" >/dev/null 2>&1 || {
          rm -f "$tmp"; pb_warn "plutil -replace failed at $k"; return 2; }
        idx=0
      fi
    else
      "$PB_PLUTIL" -replace "$k" -json "$v" "$tmp" >/dev/null 2>&1 || {
  ...
  [ "$seeded" = 1 ] && "$PB_PLUTIL" -remove _pbseed "$tmp" >/dev/null 2>&1
  ...
  pb_json_ok "$tmp" || { rm -f "$tmp"; pb_warn "write produced invalid JSON — $f untouched."; return 2; }
  if [ "$mode" = "append" ]; then
    cur="$(pb_settings_get "$tmp" "$k.$idx" "$fmt" 2>/dev/null)" || cur=""
  else
    cur="$(pb_settings_get "$tmp" "$k" "$fmt" 2>/dev/null)" || cur=""
  fi
  [ "$cur" = "$norm" ] || { rm -f "$tmp"; pb_warn "read-back mismatch at $k (want [$norm] got [$cur])"; return 2; }

  mv -f "$tmp" "$f" 2>/dev/null || { rm -f "$tmp"; pb_warn "could not replace $f"; return 2; }
  return 0
}

# ── pb_settings_ensure_parents <file> <keypath> ──────────────────────────────────────────────
...
pb_settings_ensure_parents() {
  ...
  while [ "$i" -lt $((n - 1)) ]; do
    ...
    "$PB_PLUTIL" -extract "$prefix" json -o - "$f" >/dev/null 2>&1 && continue
    case "$next" in
      ''|*[!0-9]*) : ;;
      *) pb_warn "keypath '$k': missing ancestor '$prefix' would have to be an array — create it first."; return 3 ;;
    esac
    "$PB_PLUTIL" -replace "$prefix" -json '{}' "$f" >/dev/null 2>&1 || {
      pb_warn "could not create ancestor '$prefix'"; return 2; }
  done
  return 0
}

# ═════════════════════════════════════════════════════════════════════════════════════════════
# HOOK WIRING — computed here, WRITTEN by pb_settings_merge. Two envelopes, one entry point each.
# ═════════════════════════════════════════════════════════════════════════════════════════════

# pb_hook_present <file> <event> <command> — parse-based membership. Never a grep of the file.
pb_hook_present() {
  local f="${1:-}" ev="${2:-}" cmd="${3:-}" i=0 j c
  [ -f "$f" ] || return 1
  while [ "$i" -lt 64 ] && "$PB_PLUTIL" -extract "hooks.$ev.$i" json -o - "$f" >/dev/null 2>&1; do
    j=0
    while [ "$j" -lt 64 ]; do
      c="$("$PB_PLUTIL" -extract "hooks.$ev.$i.hooks.$j.command" raw -o - "$f" 2>/dev/null)" || break
      [ "$c" = "$cmd" ] && return 0
      j=$((j + 1))
    done
    i=$((i + 1))
...
# pb_hook_wire <settings.json> <Event> <matcher> <command> [timeout]
...
pb_hook_wire() {
  ...
  [ -n "$f" ] && [ -n "$ev" ] && [ -n "$cmd" ] || { pb_warn "pb_hook_wire: need <file> <event> <matcher> <command>"; return 2; }
  pb_hook_present "$f" "$ev" "$cmd" && return 0
  entry="{\"type\":\"command\",\"command\":\"$(pb_json_escape "$cmd")\",\"timeout\":$to}"

  if [ "$(pb_settings_type "$f" "hooks.$ev")" != "array" ]; then
    pb_settings_merge "$f" "hooks.$ev" "[{\"matcher\":\"$(pb_json_escape "$mt")\",\"hooks\":[$entry]}]"
    return $?
  fi
  while [ "$i" -lt 64 ] && "$PB_PLUTIL" -extract "hooks.$ev.$i" json -o - "$f" >/dev/null 2>&1; do
    m="$("$PB_PLUTIL" -extract "hooks.$ev.$i.matcher" raw -o - "$f" 2>/dev/null)" || m=""
    if [ "$m" = "$mt" ] && [ "$found" -lt 0 ]; then found=$i; fi
    i=$((i + 1))
  done
  if [ "$found" -ge 0 ]; then
    pb_settings_merge "$f" "hooks.$ev.$found.hooks" "$entry" append
    return $?
  fi
  group="{\"matcher\":\"$(pb_json_escape "$mt")\",\"hooks\":[$entry]}"
  pb_settings_merge "$f" "hooks.$ev" "$group" append
}

# pb_hook_unwire <settings.json> <command-prefix> — remove every hook whose command starts with
...
pb_hook_unwire() {
  ...
  for ev in $(pb_hook_events "$f"); do
    i=63
    while [ "$i" -ge 0 ]; do
      if "$PB_PLUTIL" -extract "hooks.$ev.$i" json -o - "$f" >/dev/null 2>&1; then
        j=63
        while [ "$j" -ge 0 ]; do
          c="$("$PB_PLUTIL" -extract "hooks.$ev.$i.hooks.$j.command" raw -o - "$f" 2>/dev/null)" || c=""
          case "$c" in
            "$pre"*) [ -n "$c" ] && { "$PB_PLUTIL" -remove "hooks.$ev.$i.hooks.$j" "$f" >/dev/null 2>&1 && changed=1; } ;;
          esac
          j=$((j - 1))
        done
        n="$("$PB_PLUTIL" -extract "hooks.$ev.$i.hooks" json -o - "$f" 2>/dev/null)" || n=""
        [ "$n" = "[]" ] && "$PB_PLUTIL" -remove "hooks.$ev.$i" "$f" >/dev/null 2>&1
      fi
      i=$((i - 1))
    done
    n="$("$PB_PLUTIL" -extract "hooks.$ev" json -o - "$f" 2>/dev/null)" || n=""
    [ "$n" = "[]" ] && "$PB_PLUTIL" -remove "hooks.$ev" "$f" >/dev/null 2>&1
  done
  [ "$changed" = 1 ] && pb_json_ok "$f"
  return 0
}

# pb_hook_events <file> — the event names present under .hooks, one per line.
pb_hook_events() {
  local f="${1:-}" jq
  [ -f "$f" ] || return 0
  jq="$(pb_jq)" || jq=""
  ...
  for e in SessionStart SessionEnd UserPromptSubmit PreToolUse PostToolUse Stop SubagentStop \
    ...
    "$PB_PLUTIL" -extract "hooks.$e" json -o - "$f" >/dev/null 2>&1 && printf '%s\n' "$e"
  done
  return 0
}

# pb_copilot_hook_wire <hooks.json> <Event> <matcher> <command> [timeoutSec]
...
pb_copilot_hook_wire() {
  local f="${1:-}" ev="${2:-}" mt="${3:-}" cmd="${4:-}" to="${5:-10}"
  local entry i=0 b
  [ -n "$f" ] && [ -n "$ev" ] && [ -n "$cmd" ] || { pb_warn "pb_copilot_hook_wire: need <file> <event> <matcher> <command>"; return 2; }
  pb_settings_merge "$f" "version" "1" || return 2
  while [ "$i" -lt 64 ]; do
    b="$("$PB_PLUTIL" -extract "hooks.$ev.$i.bash" raw -o - "$f" 2>/dev/null)" || break
    [ "$b" = "$cmd" ] && return 0
    i=$((i + 1))
  done
  if [ -n "$mt" ]; then
    entry="{\"type\":\"command\",\"bash\":\"$(pb_json_escape "$cmd")\",\"timeoutSec\":$to,\"matcher\":\"$(pb_json_escape "$mt")\"}"
  else
    entry="{\"type\":\"command\",\"bash\":\"$(pb_json_escape "$cmd")\",\"timeoutSec\":$to}"
  fi
  if [ "$(pb_settings_type "$f" "hooks.$ev")" = "array" ]; then
    pb_settings_merge "$f" "hooks.$ev" "$entry" append
  else
    pb_settings_merge "$f" "hooks.$ev" "[$entry]"
  fi
}
...
# render pb_ctx_advisory UNCONDITIONALLY; it is silent when it has nothing to say, which is the
# only correct silence. `bash pb-lib.sh --selftest` runs the no-git arm.
# ═════════════════════════════════════════════════════════════════════════════════════════════

# pb_ctx_pct <session_id> → an integer 0-100, or nothing.
...
pb_ctx_pct() {
  ...
  if [ -n "$jq" ]; then
    ...
    pct="$("$PB_PLUTIL" -extract used_pct raw -o - "$tf" 2>/dev/null)" || pct=""
    ts="$("$PB_PLUTIL" -extract ts raw -o - "$tf" 2>/dev/null)" || ts=""
  fi
  ...
  age=$((now - ts))
  [ "$age" -lt "$PB_CTX_MAX_AGE" ] || return 0
  [ "$pct" -ge 0 ] && [ "$pct" -le 100 ] || return 0
  printf '%s' "$pct"
}

# pb_ctx_advisory <session_id> → one line of advice, or nothing. NO git anywhere in this path.
pb_ctx_advisory() {
  local pct t low
  pct="$(pb_ctx_pct "${1:-}")" || pct=""
  [ -n "$pct" ] || return 0
  t="$PB_CTX_T"
...
# pb_json <payload> <dotted.path> → scalar or empty.
...
# pb_emit_ctx <event> <text> — additionalContext. Advisory; never blocks.
...
# pb_git <dir> <args…> — git confined to <dir>, silent, never fails the caller.
# Its rc IS git's rc: pb_trunk's second arm is `rev-parse --verify`, and a wrapper that always
...
pb_git() { local d="${1:-.}"; [ $# -gt 0 ] && shift; git -C "$d" "$@" 2>/dev/null; }

# pb_count — count lines on stdin, always printing exactly ONE integer.
...
pb_count() { local n; n="$(grep -c . 2>/dev/null)" || true; case "${n:-0}" in ''|*[!0-9]*) n=0 ;; esac; printf '%s' "$n"; }

# pb_trunk <dir> → a REMOTE trunk ref, or empty.
...
pb_trunk() {
  ...
  for r in origin/HEAD origin/main origin/master; do
    v="$(pb_git "${1:-.}" rev-parse --abbrev-ref "$r")"
    [ -n "$v" ] && pb_git "${1:-.}" rev-parse --verify -q "$v" >/dev/null && { printf '%s' "$v"; return 0; }
  done
  return 0
}
...
# Every jq-dependent case is run TWICE: once as the machine is, once under PB_NO_JQ=1.
# ═════════════════════════════════════════════════════════════════════════════════════════════
pb__t=0; pb__f=0
pb_ok()   { pb__t=$((pb__t+1)); printf '  ok   %s\n' "$1"; }
pb_bad()  { pb__t=$((pb__t+1)); pb__f=$((pb__f+1)); printf '  FAIL %s\n' "$1"; [ -n "${2:-}" ] && printf '       %s\n' "$2"; }
pb_is()   { if [ "$2" = "$3" ]; then pb_ok "$1"; else pb_bad "$1" "want [$3] got [$2]"; fi; }
...
pb_selftest() {
  ...
  printf '{}\n' > "$T/c1.json"
  "$PB_PLUTIL" -replace statusLine -json '{"type":"command","command":"/x.sh"}' "$T/c1.json" >/dev/null 2>&1
  rc=$?
  if [ "$rc" = 0 ]; then
    pb_bad "C1 pre-fix arm is RED (plutil -replace refuses an empty root dict)" \
           "plutil now ACCEPTS it — the seed dance is no longer attributable to a measured defect."
  else
    pb_ok "C1 pre-fix arm is RED (plutil -replace on {} exits $rc — the defect the seed dance repairs)"
  fi
  "$PB_PLUTIL" -insert statusLine -json '{"a":1}' "$T/c1.json" >/dev/null 2>&1
  [ $? -ne 0 ] && pb_ok "C1 pre-fix arm: -insert is NOT the fix either" || pb_bad "C1: -insert unexpectedly worked"

  # ── 2-6. run the whole settings block on BOTH engines ─────────────────────────────────────
  for arm in jq nojq; do
    if [ "$arm" = nojq ]; then export PB_NO_JQ=1; else unset PB_NO_JQ; fi
    if [ "$arm" = jq ] && ! pb_have_jq; then printf '  --   [jq arm skipped: no jq on this box]\n'; continue; fi
    PB_BACKED_UP=""

    # 2. C1 POST-FIX: the same empty root dict, through the one writer.
    printf '{}\n' > "$T/s.json"
    pb_settings_merge "$T/s.json" statusLine '{"type":"command","command":"/tmp/sl.sh"}'
    pb_is "[$arm] C1 green: merge into a literal {} succeeds" "$?" "0"
    pb_is "[$arm] C1 green: read-back through plutil" "$(pb_settings_get "$T/s.json" statusLine.command raw)" "/tmp/sl.sh"
    pb_is "[$arm] seed key removed" "$(pb_settings_get "$T/s.json" _pbseed raw 2>/dev/null)" ""

    # 3. IDEMPOTENT: the second identical call must not touch the file at all.
    A="$(shasum -a 256 "$T/s.json" | cut -d' ' -f1)"
    pb_settings_merge "$T/s.json" statusLine '{"type":"command","command":"/tmp/sl.sh"}' >/dev/null 2>&1
    B="$(shasum -a 256 "$T/s.json" | cut -d' ' -f1)"
    pb_is "[$arm] idempotent: second merge is a no-op" "$A" "$B"

    # 4. ADDITIVE: an unrelated key the user owns survives, and so does its value.
    printf '%s\n' '{"model":"claude-opus-5","env":{"FOO":"bar"}}' > "$T/a.json"
    pb_settings_merge "$T/a.json" statusLine '{"type":"command","command":"/tmp/sl.sh"}' >/dev/null 2>&1
    pb_is "[$arm] additive: unrelated scalar survives" "$(pb_settings_get "$T/a.json" model raw)" "claude-opus-5"
    pb_is "[$arm] additive: unrelated nested value survives" "$(pb_settings_get "$T/a.json" env.FOO raw)" "bar"
    ...
    pb_hook_wire "$T/h.json" Stop "" "/pb/pb-stop.sh" >/dev/null 2>&1
    pb_is "[$arm] hook: user's hook still first" "$(pb_settings_get "$T/h.json" hooks.Stop.0.hooks.0.command raw)" "/mine/existing.sh"
    pb_is "[$arm] hook: ours appended into the same matcher group" "$(pb_settings_get "$T/h.json" hooks.Stop.0.hooks.1.command raw)" "/pb/pb-stop.sh"
    A="$(shasum -a 256 "$T/h.json" | cut -d' ' -f1)"
    pb_hook_wire "$T/h.json" Stop "" "/pb/pb-stop.sh" >/dev/null 2>&1
    B="$(shasum -a 256 "$T/h.json" | cut -d' ' -f1)"
    pb_is "[$arm] hook: re-wiring is a no-op" "$A" "$B"
    pb_hook_wire "$T/h.json" PreToolUse "Write|MultiEdit" "/pb/pb-guard-write.sh" >/dev/null 2>&1
    pb_is "[$arm] hook: a new event is created as an ARRAY, not a dict" "$(pb_settings_type "$T/h.json" hooks.PreToolUse)" "array"
    pb_is "[$arm] hook: new matcher group carries the matcher" "$(pb_settings_get "$T/h.json" hooks.PreToolUse.0.matcher raw)" "Write|MultiEdit"
    pb_hook_wire "$T/h.json" Stop "OTHER" "/pb/other.sh" >/dev/null 2>&1
    pb_is "[$arm] hook: a different matcher makes a NEW group" "$(pb_settings_get "$T/h.json" hooks.Stop.1.hooks.0.command raw)" "/pb/other.sh"
    pb_hook_unwire "$T/h.json" "/pb/" >/dev/null 2>&1
    pb_is "[$arm] unwire: ours gone" "$(pb_settings_get "$T/h.json" hooks.Stop.0.hooks.1.command raw 2>/dev/null)" ""
    pb_is "[$arm] unwire: the user's survives" "$(pb_settings_get "$T/h.json" hooks.Stop.0.hooks.0.command raw)" "/mine/existing.sh"

    # 6. COPILOT: a fresh envelope on a machine with no ~/.copilot at all.
    rm -rf "$T/copilot"
    pb_copilot_hook_wire "$T/copilot/hooks/00-lifecycle.json" Stop "" "/pb/pb-stop.sh" >/dev/null 2>&1
    pb_is "[$arm] copilot: version pinned" "$(pb_settings_get "$T/copilot/hooks/00-lifecycle.json" version raw)" "1"
    pb_is "[$arm] copilot: bash key, not command" "$(pb_settings_get "$T/copilot/hooks/00-lifecycle.json" hooks.Stop.0.bash raw)" "/pb/pb-stop.sh"
    pb_is "[$arm] copilot: event is an ARRAY" "$(pb_settings_type "$T/copilot/hooks/00-lifecycle.json" hooks.Stop)" "array"
    A="$(shasum -a 256 "$T/copilot/hooks/00-lifecycle.json" | cut -d' ' -f1)"
    pb_copilot_hook_wire "$T/copilot/hooks/00-lifecycle.json" Stop "" "/pb/pb-stop.sh" >/dev/null 2>&1
    B="$(shasum -a 256 "$T/copilot/hooks/00-lifecycle.json" | cut -d' ' -f1)"
    pb_is "[$arm] copilot: re-wiring is a no-op" "$A" "$B"

    # 7. REFUSAL at the chokepoint — and the file must be untouched.
    printf '%s\n' '{"model":"x"}' > "$T/r.json"
    A="$(shasum -a 256 "$T/r.json" | cut -d' ' -f1)"
    pb_settings_merge "$T/r.json" permissions.allow '["Bash(*)"]' >/dev/null 2>&1
    pb_is "[$arm] refuses to write permissions" "$?" "2"
    B="$(shasum -a 256 "$T/r.json" | cut -d' ' -f1)"
    pb_is "[$arm] refused write left the file untouched" "$A" "$B"
    pb_settings_merge "$T/settings.local.json" model '"x"' >/dev/null 2>&1
    pb_is "[$arm] refuses settings.local.json entirely" "$?" "2"

    # 8b. A file that PARSES but is not JSON text (an XML plist) is never touched either.
    printf '%s\n' '{"model":"x"}' > "$T/xml.json"; "$PB_PLUTIL" -convert xml1 "$T/xml.json" >/dev/null 2>&1
    A="$(shasum -a 256 "$T/xml.json" | cut -d' ' -f1)"
    pb_settings_merge "$T/xml.json" statusLine '{"a":1}' >/dev/null 2>&1
    pb_is "[$arm] refuses an XML plist wearing a .json name" "$?" "2"
    pb_is "[$arm] XML plist untouched" "$A" "$(shasum -a 256 "$T/xml.json" | cut -d' ' -f1)"

    # 8. A file that is NOT valid JSON is never touched.
    printf '%s\n' 'not json at all {' > "$T/bad.json"
    A="$(shasum -a 256 "$T/bad.json" | cut -d' ' -f1)"
    pb_settings_merge "$T/bad.json" statusLine '{"a":1}' >/dev/null 2>&1
    pb_is "[$arm] refuses a non-JSON settings file" "$?" "2"
    pb_is "[$arm] non-JSON file untouched" "$A" "$(shasum -a 256 "$T/bad.json" | cut -d' ' -f1)"
  done
  unset PB_NO_JQ
  ...
  PB_TELEMETRY_DIR="$T/tel"; mkdir -p "$PB_TELEMETRY_DIR"
  sid="SELFTEST-SID"
  printf '{"ts":%s,"session_id":"%s","window":1000000,"used_pct":82,"input_tokens":820000}\n' "$(date +%s)" "$sid" > "$PB_TELEMETRY_DIR/$sid.json"
  out="$( cd "$T/notarepo" && PATH="$T/nogit/bin" pb_ctx_advisory "$sid" )"
  case "$out" in
    CONTEXT\ 82%*) pb_ok "C3: 82% advisory FIRES with no git on PATH and cwd not a repo" ;;
    *)             pb_bad "C3: 82% advisory lost outside a git repo" "got [$out]" ;;
  esac
  printf '{"ts":%s,"used_pct":12}\n' "$(date +%s)" > "$PB_TELEMETRY_DIR/$sid.json"
  out="$( cd "$T/notarepo" && PATH="$T/nogit/bin" pb_ctx_advisory "$sid" )"
  pb_is "C3 polarity: 12% stays SILENT" "$out" ""
  printf '{"ts":1,"used_pct":82}\n' > "$PB_TELEMETRY_DIR/$sid.json"
  pb_is "C3: stale telemetry stays SILENT" "$(pb_ctx_advisory "$sid")" ""
  printf '{"ts":%s,"used_pct":null}\n' "$(date +%s)" > "$PB_TELEMETRY_DIR/$sid.json"
  pb_is "C3: a null used_pct is NOT read as 0%" "$(pb_ctx_advisory "$sid")" ""
  printf '{"ts":%s,"used_pct":45.7}\n' "$(date +%s)" > "$PB_TELEMETRY_DIR/$sid.json"
  pb_is "C3: a FLOAT fill truncates rather than being discarded" "$(pb_ctx_pct "$sid")" "45"
  rm -f "$PB_TELEMETRY_DIR/$sid.json"
  pb_is "C3: absent telemetry stays SILENT" "$(pb_ctx_advisory "$sid")" ""
  pb_is "C3: no session id stays SILENT" "$(pb_ctx_advisory "")" ""

  # ── 10. D4: escaping. The exact shape that made the receipt unparseable. ──────────────────
  out="$(pb_json_escape 'kitty says "unknown action" and exits 0 \ ok')"
  printf '{"note":"%s"}\n' "$out" > "$T/d4.json"
  if pb_json_ok "$T/d4.json"; then pb_ok "D4: a quoted+backslashed note still yields valid JSON"
  else pb_bad "D4: escaping failed" "$(cat "$T/d4.json")"; fi
  pb_is "D4: the note round-trips through plutil unchanged" \
        "$(pb_settings_get "$T/d4.json" note raw)" 'kitty says "unknown action" and exits 0 \ ok'
  printf '{"note":"he said "hi""}\n' > "$T/d4bad.json"
  if pb_json_ok "$T/d4bad.json"; then pb_bad "D4 control: the validator passed MALFORMED json"; else pb_ok "D4 control: the validator rejects an unescaped quote"; fi

  # ── 11. pb_count: exactly one integer, even on empty input. ───────────────────────────────
  pb_is "pb_count on empty stdin prints exactly one 0" "$(printf '' | pb_count)" "0"
  pb_is "pb_count on 3 lines" "$(printf 'a\nb\nc\n' | pb_count)" "3"

  # ── 12. pb_trunk: a repo with no remote must answer nothing, not 'master'. ────────────────
  if command -v git >/dev/null 2>&1 && git -C "$T" init -q "$T/norepo" >/dev/null 2>&1; then
    pb_is "pb_trunk: no remote ⇒ empty (never a local branch)" "$(pb_trunk "$T/norepo")" ""
  else
    printf '  --   [pb_trunk case skipped: git unavailable — which is itself the fresh-Mac state]\n'
  fi
...
if [ "${BASH_SOURCE[0]:-x}" = "${0:-y}" ]; then
  case "${1:-}" in
    --selftest) pb_selftest; exit $? ;;
    --version)  printf 'pb-lib %s\n' "$PB_LIB_VERSION"; exit 0 ;;
    *)          printf 'pb-lib.sh is a library. Source it, or run: bash pb-lib.sh --selftest\n' >&2; exit 0 ;;
  esac
fi

# ---------------------------------------------------------------------------------------------
# pb_defaults_home_ok — refuse a `defaults` write when $HOME is not this user's REAL home.
#
...
# cross-home install: PB_ALLOW_FOREIGN_DEFAULTS=1.
pb_defaults_home_ok() {
  [ "${PB_ALLOW_FOREIGN_DEFAULTS:-0}" = 1 ] && return 0
  local real
  ...
  [ "$real" = "${HOME%/}" ] && return 0
  pb_warn "defaults writes would escape this sandboxed HOME and hit the REAL domain ($real). Refusing. Set PB_ALLOW_FOREIGN_DEFAULTS=1 only if that is genuinely what you want."
  return 1
}
